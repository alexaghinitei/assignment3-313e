...
def prepare_game():
    ...
    # logic, if num args = 1, nothing is parsed pick a random word, if len is 2, either a seed or a secret word
    if len(sys.argv) == 1: # i.e pick random
        secret_word = random.choice(secret_words)

    elif len(sys.argv) == 2:
        if sys.argv[1].isalpha() and len(sys.argv[1]) == 5 and sys.argv[1].islower(): #inputting manual secret word
            secret_word = sys.argv[1]
        elif sys.argv[1].isnumeric():
            random.seed(int(sys.argv[1])) # setting the seed
            secret_word = random.choice(secret_words)
        else:
            raise ValueError()
    else:
...
def get_feedback(secret_word, guessed_word):
    ...
    # Creates a list of letter counts to keep track of letters
    letter_counts = {}
    ...
    # Marks all of the correct letters
    for i in range(NUM_LETTERS):
        if guessed_word[i] == secret_word[i]:
            feedback[i] = CORRECT_COLOR
            letter_counts[guessed_word[i]] -= 1
    for i in range(NUM_LETTERS):
        if feedback[i] is None: # letter isnt correct already
            # makes sure it isnt showing yellow for an already used letter or 
